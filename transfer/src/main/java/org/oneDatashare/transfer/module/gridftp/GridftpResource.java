...
public class GridftpResource extends Resource<GridftpSession, GridftpResource> {
    ...
    public Mono<org.oneDatashare.transfer.model.core.Stat> stat() {
        return initialize().flatMap(GridftpResource::onStat);
    }

    public Mono<org.oneDatashare.transfer.model.core.Stat> onStat() {
        return getSession().client
                .listFiles(getSession().endpoint.getId(), getPath(), showHidden, offset, limit, orderedBy, filter)
                .map(
                    fileList -> {
                    org.oneDatashare.transfer.model.core.Stat stat = new org.oneDatashare.transfer.model.core.Stat();
                    org.oneDatashare.transfer.model.core.Stat filesStat[] = new org.oneDatashare.transfer.model.core.Stat[fileList.getData().size()];
                    for(int i = 0; i < fileList.getData().size(); i++){
                        org.oneDatashare.transfer.model.core.Stat tempStat = new org.oneDatashare.transfer.model.core.Stat();
    ...
    private org.oneDatashare.transfer.model.core.Stat mDataToStat(Metadata data) {
        org.oneDatashare.transfer.model.core.Stat stat = new org.oneDatashare.transfer.model.core.Stat(data.getName());
...
//    return slices.doOnNext(dbxDrain::drain).doFinally(s -> dbxDrain.finish());
    ...
    @Override
    public Mono<org.oneDatashare.transfer.model.core.Stat> getTransferStat() {
        return null;
    }
